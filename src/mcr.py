...
def run_all():
    ...
    trim = functions.run_trimal(aln)
    #fasttree = functions.run_fasttree(trim)
    iqtree = functions.run_iqtree(trim, 'LG+R10')

    functions.create_table(output_dir
                           + '/sqlite_metatdata', 'mcr_canon')
    functions.insert_all_file(output_dir
                              + '/sqlite_metatdata', 'mcr_canon', output_dir
                              + '/canon/results.map')
    functions.create_table(output_dir
                           + '/sqlite_metatdata', 'mcr_prev')
    functions.insert_all_file(output_dir
                              + '/sqlite_metatdata', 'mcr_prev', output_dir
                              + '/prevalence/results.blast.map')
    functions.make_prev_db(output_dir
                           + '/sqlite_metatdata', 'prev_meta')
    functions.create_table(output_dir
                           + '/sqlite_metatdata', 'mcr_nrdb')
    functions.insert_all_file(output_dir
                              + '/sqlite_metatdata', 'mcr_nrdb', output_dir
                              + '/nrdb/results.blast.map')
    functions.create_table(output_dir
                           + '/sqlite_metatdata', 'mcr_uba')
    functions.insert_all_file(output_dir
                              + '/sqlite_metatdata', 'mcr_uba', output_dir
    ...
    tree2 = functions.annotate(tree, 'mcr' ,output_dir
                               + '/sqlite_metatdata')
